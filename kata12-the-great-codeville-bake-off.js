  ...
  for (let recipe of recipes) {
    ...
    const ingredientCheck = function(bakery) {
      for (let ingredient of bakery) {
        if (ingredient === ingredients[0] || ingredient === ingredients[1]) {
          return ingredient;
        }
      }
    };
    if (ingredientCheck(bakeryA) === ingredients[0] && ingredientCheck(bakeryB) === ingredients[1]) {
      return name;
    } else if (ingredientCheck(bakeryA) === ingredients[1] && ingredientCheck(bakeryB) === ingredients[0]) {
      return name;
    }
  }
};
